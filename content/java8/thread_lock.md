...
### 对比`synchronized`, ReentrantLock的一些高级功能
...
ReentrantLock默认的构造函数是创建的`非公平锁`，可以通过参数`true`设为`公平锁`，但公平锁表现的性能不是很好。

synchronized是非公平锁（因为synchronized是不公平竞争，后来的线程可能先得到锁，进而可能导致先到的线程持续饥饿，非公平竞争在很大程度上提升了synchronized吞吐率），可以重入
