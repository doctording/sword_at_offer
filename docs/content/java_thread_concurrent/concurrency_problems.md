...
1. `高并发`、`任务执行时间短`的业务：线程池线程数可以设置为CPU核数+1，减少线程上下文切换
...
3. 并发高、且业务执行时间长的业务：解决这种类型任务的关键不在于线程池而在于整体架构的设计，看看这些业务里面某些数据是否能`缓存`；增加服务器；需要分析业务执行时间长的问题，看看能不能使用中间件对任务进行拆分和解耦。
